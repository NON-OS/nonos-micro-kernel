...
| `Stark.Fold.eval_split` / `evens_length` / `odds_length` | the FRI split identity `f(x) = f_even(x^2) + x f_odd(x^2)` and the coefficient halving that makes folding shrink the degree; `userland/stark_proofs` `fri_tests.rs` |
| `Stark.Fold.the_honest_fold_reaches_a_constant` | the concrete form of the FRI conclusion on the real polynomial: a word of length at most `2^r` folded `r` times is a single coefficient, binding `Fri.enough_folds_reach_a_constant`'s abstract bound to the actual iterated fold |
...
| `Stark.RunningSum.final_is_total` / `conservation` / `follows_pins_the_trace` | the additive accumulator value conservation rests on: the column ends at its total (boundary check reads the sum), and the per-row transition constraints pin the trace given its start; gives `sum(inputs) = sum(outputs) + fee` with no inflation, the shielded-pool value gate |
| `Stark.NullifierSet.a_recorded_nullifier_cannot_be_respent` / `spend_only_grows` | double-spend soundness as a state machine over the spent set: a fresh nullifier is recorded, a spent one is rejected, the set only grows, and a recorded nullifier can never be spent again; no note is spent twice, the pool's balance-safety guard |
| `Stark.Pool.every_reachable_pool_is_solvent` | the solvency capstone: the held balance exactly backs the shielded total after any sequence of deposits and withdrawals, with withdraw gated through the nullifier guard; the on-chain solvency invariant an auditor demands, proven over all traces |
| `Stark.AssociationSet.an_excluded_deposit_cannot_pass` / `the_registry_only_grows` | decentralized compliance without a gatekeeper: a withdrawal accepted against a set that excludes a commitment cannot be a spend of it (choosing a clean set proves clean funds without deanonymizing), and the registry of published sets is append-only and permissionless so no publisher can censor another or invalidate an old proof |
| `Stark.Commitment.commitment_binds_value` / `distinct_values_distinct_commitments` | the note commitment fixes the amount: under the hash's injectivity (its collision resistance, a named hypothesis discharged by the Poseidon KATs) equal commitments force equal values, so a deposit cannot be reopened to a different amount |
| `Stark.FeeRouter.route_conserves` / `an_accepted_fee_is_within_cap` | revenue integrity: the three fee shares sum to exactly the fee (no mint, no leak), and any fee governance sets is at or below the hard cap, so it cannot rug users by raising the rate |
| `Stark.RootWindow.a_pushed_root_is_accepted` / `the_window_never_exceeds_the_cap` | the recent-roots window so proofs and deposits do not race: a freshly committed root is accepted, the buffer is bounded by its cap and ages deterministically, and only committed roots verify |
| `Stark.Staking.rewards_never_exceed_fees` | the no-inflation invariant of staking, over all traces: total rewards distributed never exceed total fees funded, so yield is a claim on real revenue and nothing is minted |
...
| `Stark.Polynomial.root_bound` | the numeric Schwartz-Zippel core, proved outright: a polynomial vanishing at as many distinct points as its length is zero, so a nonzero polynomial has fewer roots than its degree |
| `Stark.Extension.low_degree_extension_is_unique` / `distinct_low_degree_polys_disagree` | the Reed-Solomon uniqueness the proximity test rests on: two polynomials of degree below n agreeing on n distinct points are equal, so a codeword is pinned to one low-degree polynomial once it agrees past the degree; follows from `root_bound` on the difference |
...
| `Stark.Constraint.the_quotient_is_pinned` | the prover cannot wiggle the witness: any two low-degree quotients for the same constraint agree everywhere off the domain, since the zerofier is nonzero there and cancels, so the object the FRI test checks is determined |
| `Stark.Instance.honest_run_has_a_quotient` / `tampered_run_has_no_quotient` | a worked bit-gate AIR (`C = W(W-1)`) with concrete traces, so soundness is not vacuous: an honest bit column is accepted and a column with a non-bit row is rejected, both through the abstract soundness; matches the bit-decomposition gate in `userland/stark_proofs` `air_tests.rs` |
