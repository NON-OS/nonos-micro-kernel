---
title: "prøve: A Verifiable-Compute Language Proven by a Transparent STARK"
...
date: "2026"
abstract: |
  We present prøve, a small language for verifiable computation built into NØNOS.
  A prøve program compiles to a fixed-width register machine over the Goldilocks
  ...
  the kernel already runs. Three choices distinguish prøve from a general zero
...
cryptographic language as the rest of your system. That is the problem prøve
solves inside NØNOS. A prøve program is ordinary straight-line field arithmetic
...
and that prover, and in particular one construction: because prøve names its
...
We bought three things. The first is a shared field. Every prøve value is a
...
tree, and a prøve trace is already in the representation the prover consumes; no
serialization, no re-encoding, no second field to keep in sync. The second is an
auditable core. prøve has no data-dependent control flow, so the sequence of
...
We gave up generality. prøve does not run arbitrary programs; it runs the programs
...
A prøve program is a sequence of statements over field values. The concrete syntax
...
(`userland/nonos_prove/src/lang/lex.rs`). The grammar, lowest precedence first,
is:
...
prøve does not need it, because its register indices are compile-time constants.
...
paper [@nonosverification], and prøve treats it as a component. The seam is the
whole point: prøve emits an ordinary AIR over the shared field, and the STARK does
the rest. The soundness of a prøve proof is the soundness of that STARK applied to
...
Proving is work, and prøve prices it. The cost driver is the trace area, rows times
...
`cargo run --release --example measure` in `userland/nonos_prove_proofs`.
...
`userland/nonos_prove_proofs` (`cargo test`). An honest run of a program touching
...
verifier, which prøve does not offer today since all inputs are public; and the
on-chain settlement of the NOX fee. None of these is in the tree, and no claim
above depends on them.

# Related work

prøve sits in the family of STARK-based provable virtual machines. Cairo
...
full generality for a large interpreter circuit and a memory argument. prøve makes
...
prøve shows that a verifiable-compute language can be small, legible, and honest
