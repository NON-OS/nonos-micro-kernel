...
impl Sv39 {
    pub const fn vpn(va: usize, level: usize) -> usize {
        (va >> (12 + level * VPN_BITS)) & VPN_MASK
    }
    pub const fn page_offset(va: usize) -> usize {
        va & (PAGE_SIZE - 1)
    }
    pub const fn make_pte(ppn: u64, flags: PteFlags) -> u64 {
        (ppn << 10) | flags.bits()
    }
    pub const fn pte_ppn(pte: u64) -> u64 {
        (pte >> 10) & ((1 << 44) - 1)
    }
    pub const fn pte_flags(pte: u64) -> PteFlags {
        PteFlags::from_bits(pte & 0xff)
    }
    ...
    pub const fn is_aligned(addr: usize, level: usize) -> bool {
...
pub const fn kernel_va_start() -> usize {
    0xffff_ffc0_0000_0000
}
pub const fn kernel_va_end() -> usize {
    usize::MAX
}
pub const fn user_va_start() -> usize {
    0
}
pub const fn user_va_end() -> usize {
    0x0000_003f_ffff_ffff
}
pub const fn is_kernel_va(va: usize) -> bool {
    va >= kernel_va_start()
}
