...
pub(super) fn input_fields(
    ...
    if header.is_writable() && header.is_executable() {
        return Err(ElfError::WXViolation);
    }
    if header.p_align > 1 && !header.p_align.is_power_of_two() {
    ...
    let file_size =
        usize::try_from(header.p_filesz).map_err(|_| ElfError::SegmentDataOutOfBounds)?;
    let seg_size = usize::try_from(header.p_memsz).map_err(|_| ElfError::SegmentDataOutOfBounds)?;
    let file_offset =
        usize::try_from(header.p_offset).map_err(|_| ElfError::SegmentDataOutOfBounds)?;
