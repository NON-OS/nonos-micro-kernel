...
pub(super) fn ensure_target_range(image: &ElfImage, addr: u64, reloc_type: u32) -> Result<(), ElfError> {
    let size = match reloc_type {
        reloc_type::R_X86_64_NONE | reloc_type::R_X86_64_COPY => 0,
        reloc_type::R_X86_64_64 | reloc_type::R_X86_64_GLOB_DAT | reloc_type::R_X86_64_JUMP_SLOT
        | reloc_type::R_X86_64_RELATIVE | reloc_type::R_X86_64_IRELATIVE => 8,
        reloc_type::R_X86_64_PC32 | reloc_type::R_X86_64_GOT32 | reloc_type::R_X86_64_PLT32
        | reloc_type::R_X86_64_GOTPCREL | reloc_type::R_X86_64_32 | reloc_type::R_X86_64_32S => 4,
    ...
    if size == 0 || in_segment(image, addr, size, false) { return Ok(()); }
    Err(ElfError::RelocationFailed)
}

pub(super) fn resolve_resolver_addr(image: &ElfImage, base: u64, addend: i64) -> Result<u64, ElfError> {
    let addr = base.checked_add_signed(addend).ok_or(ElfError::AddressOverflow)?;
    if in_segment(image, addr, 1, true) { return Ok(addr); }
    Err(ElfError::RelocationFailed)
}
