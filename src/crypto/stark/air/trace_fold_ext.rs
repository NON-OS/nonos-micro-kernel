...
impl TraceFoldExt {
    pub fn new(
        ...
        TraceFoldExt { log_layers, n_folds, x_inv, dir, final_value }
    }

    /// The witness: one `[beta, a, b]` row per layer as six base columns, the final
    /// layer carrying its pair, the rest padded.
    ...
    fn transition_impl<F: Felt>(&self, window: &[F], periodic: &[F]) -> Vec<F> {
        ...
        let (na_c0, na_c1) = (window[8], window[9]);
        let (nb_c0, nb_c1) = (window[10], window[11]);
        let sel = periodic[0];
        ...
        alloc::vec![sel * (folded_c0 - expected_c0), sel * (folded_c1 - expected_c1)]
    }
}
...
impl Air for TraceFoldExt {
    ...
    fn constraint_degree(&self) -> usize {
        ...
        // public inverse point, then the selector.
        4
    }
    ...
    fn boundary(&self) -> Vec<(usize, usize, Fp)> {
        ...
        alloc::vec![
            (2, self.n_folds, self.final_value.c0),
            (3, self.n_folds, self.final_value.c1),
        ]
    }
}
