...
pub fn stark_verify_poseidon_ext<A: AirExt>(
    ...
    if !fri_verify_poseidon_ext(&proof.fri, shift, log_n, fri_log_blowup, n_queries, grind_bits, hasher)
    {
        return false;
    }
    ...
    for qd in &proof.queries {
        ...
        for c in 0..width {
            if !verify_path(hasher, &proof.trace_roots[c], p, pack_base(qd.trace[c]), &qd.trace_paths[c]) {
                return false;
            }
        }
        ...
        for k in 0..window_size {
            ...
            for c in 0..width {
                ...
                acc = acc
                    + deep_coeffs[k * width + c] * ((Fp2::from_base(qd.trace[c]) - claimed) * inv_x_zk);
            }
        }
