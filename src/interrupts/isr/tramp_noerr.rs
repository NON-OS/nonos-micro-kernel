...
//! Trampoline for a vector the CPU enters without an error code, so CS sits
//! at `[rsp+8]`. The 15 GP register pushes are one `\n`-joined template to
//! keep the body a single literal.
...
macro_rules! exc_tramp_noerr {
    ($tramp:ident, $shim:ident, $handler:path) => {
        ...
        extern "C" fn $shim(frame: *const x86_64::structures::idt::InterruptStackFrame) {
            $handler(unsafe { core::ptr::read(frame) });
        }
    };
}

pub(super) use exc_tramp_noerr;
