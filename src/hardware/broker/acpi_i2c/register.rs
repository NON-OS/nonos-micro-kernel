...
use crate::arch::x86_64::acpi::aml::{enumerate_i2c_controllers, enumerate_i2c_hid};
...
/// it; the touchpad records still let the HID driver skip address probing.
pub fn register_acpi_i2c() {
    ...
    for dev in enumerate_i2c_hid() {
        if dev.slave_addr != 0 {
            register_platform_device(hid_record(&dev));
        }
    }
}
