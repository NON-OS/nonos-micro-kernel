...
/// `syscall`, with rax zero and rsp pointing at the function and argument
/// it pushed. So the child's entry is the caller's return address and
...
pub fn clone(guest: &mut Guest, frame: &ForeignFrame) -> Answer {
    ...
    if frame.rip == 0 {
        // The kernel is not yet passing the guest's return address, so
        // there is nowhere correct to start the child. Refusing beats
        // starting it at an address that is not its own.
        return Answer::value(errno::fail(errno::ENOSYS));
    }
    if stack == 0 {
