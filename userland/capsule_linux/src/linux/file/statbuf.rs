...
//!
//! The layout is Linux's and the field offsets are load-bearing: a libc
//! reads st_mode and st_size straight out of these bytes. Everything the
//! store does not know is left at zero rather than invented, except the
//! link count and the block size, where zero would be read as a broken
