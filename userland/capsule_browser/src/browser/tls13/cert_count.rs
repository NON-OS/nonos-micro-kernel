...
pub fn cert_count(body: &[u8]) -> u8 {
    ...
    while pos + 5 <= body.len() {
        ...
        let Some(cert_end) = pos.checked_add(3).and_then(|v| v.checked_add(cert_len)) else { return 0 };
        if cert_end + 2 > body.len() {
            return 0;
        }
        let ext_len = ((body[cert_end] as usize) << 8) | body[cert_end + 1] as usize;
        let Some(next) = cert_end.checked_add(2).and_then(|v| v.checked_add(ext_len)) else { return 0 };
