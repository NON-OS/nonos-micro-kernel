...
/// Both release commands carry only a resource id and padding.
pub(super) fn submit(
    q: &ControlQueue,
    type_: u32,
    fence_id: u64,
    ...
    Hdr::new(type_, fence_id).write(&mut req[..HDR_LEN]);
    ...
    let hdr = Hdr::parse(&resp).ok_or("virtio-gpu: bad release response")?;
