...
//! `prove`: run a zkolang program and prove its execution with the in-kernel
...
use nonos_zkolang::{prove_source, quote, RunError};
    ...
    if argv.len() >= 2 {
        ...
        match vfs::read_file(owner, &path, MAX_READ) {
            Ok(bytes) => match core::str::from_utf8(&bytes) {
                ...
                Err(_) => {
                    Output::new(&mut state.scrollback).writeln(b"zkolang: file is not valid UTF-8")
                }
            },
        ...
        Output::new(&mut state.scrollback)
            .writeln(b"zkolang: no file given, proving the demo program");
        report(state, DEMO);
    }
}
...
fn report(state: &mut State, src: &str) {
    ...
    out.writeln(b"zkolang: NONOS verifiable-compute language");
