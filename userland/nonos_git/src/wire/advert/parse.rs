...
///
/// A repository with no refs still has to answer, so it sends one entry named
/// `capabilities^{}` purely to carry that list. It stands for no object and is
/// skipped, or a push would read it as a branch.
