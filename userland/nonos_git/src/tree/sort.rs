...
//! directory `foo` (`foo.` < `foo/`). Getting it wrong changes the tree's hash
//! while still producing a file git can parse, so it is isolated and tested.
...
/// Whether entries are already in the required order with no duplicate name,
/// which is what a well-formed tree read off disk must satisfy.
