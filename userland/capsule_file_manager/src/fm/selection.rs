...
// Check every entry currently in view.
...
pub fn is_selected(state: &State, full_path: &str) -> bool {
    state.selected.iter().any(|p| p == full_path)
}

// The set of (full_path, is_dir) a batch action applies to: the checked
// entries when any are checked, otherwise just the entry under the cursor.
