...
//! Register phase: claim the device's legacy register window as
//! either an MMIO mapping or a PIO grant, picked from the BAR kind
//! discovery reported. `RegisterGrant` carries the live grant and
//! yields a transport-agnostic `Regs` for the init handshake.
...
pub fn grant(dev: Found, epoch: u64) -> Result<RegisterGrant, &'static str> {
    match dev.register_kind {
        ...
        _ => Err("virtio-rng: unsupported register bar"),
    }
}
