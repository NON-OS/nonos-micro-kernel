...
pub enum GalleryAction { None, Open(String), Repaint }
...
pub fn on_event(g: &mut GalleryState, ev: &InputEvent, win_w: u32, win_h: u32) -> GalleryAction {
    ...
    match ev.kind {
        InputKind::ButtonDown => match hit_tile(ev.x, ev.y, g.scroll, win_w, count) {
            Some(i) => { g.sel = i; GalleryAction::Open(g.entries[i].path.clone()) }
            None => GalleryAction::None,
        },
        InputKind::Wheel => { scroll(g, ev.delta_y, win_h, win_w, count); GalleryAction::Repaint }
...
fn scroll(g: &mut GalleryState, delta_y: i32, win_h: u32, win_w: u32, count: usize) {
    ...
    if delta_y > 0 { g.scroll = g.scroll.saturating_sub(1); }
    else { g.scroll = (g.scroll + 1).min(max); }
}

fn key(g: &mut GalleryState, code: u32, win_w: u32, count: usize) -> GalleryAction {
    if count == 0 { return GalleryAction::None; }
    let cols = grid(win_w).cols as usize;
    match code {
        KEY_LEFT => { g.sel = g.sel.saturating_sub(1); GalleryAction::Repaint }
        KEY_RIGHT => { g.sel = (g.sel + 1).min(count - 1); GalleryAction::Repaint }
        KEY_UP => { g.sel = g.sel.saturating_sub(cols); GalleryAction::Repaint }
        KEY_DOWN => { g.sel = (g.sel + cols).min(count - 1); GalleryAction::Repaint }
