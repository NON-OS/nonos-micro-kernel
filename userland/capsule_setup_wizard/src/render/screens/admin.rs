...
pub fn draw(ctx: &Context) {
    render::frame(ctx, b"Administration password", b"Used to authorise privileged actions", b"TYPE  BACKSPACE EDIT  ENTER NEXT  ESC BACK");
    let spx = ctx.stride as usize / 4;
    let (w, h) = (ctx.width, ctx.height);
    let buf = render::buffer(ctx);
    widgets::field::masked(buf, spx, w, h, render::content_x(w), 120, ctx.admin_len, widgets::field::strength_of(ctx.admin_len));
}
