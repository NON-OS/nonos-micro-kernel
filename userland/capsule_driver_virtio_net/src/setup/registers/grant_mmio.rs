...
use super::constants::PAGE_MASK;
use super::register_grant::RegisterGrant;
use crate::discover::Found;

pub(super) fn grant_mmio(dev: Found, claim_epoch: u64) -> Result<RegisterGrant, &'static str> {
    let mut out = MmioMapOut { user_va: 0, length: 0, grant_id: 0 };
    let length = (dev.register_size + PAGE_MASK) & !PAGE_MASK;
    let r =
        mk_mmio_map(dev.device_id, claim_epoch, dev.register_bar as u32, 0, 0, length, &mut out);
    if r < 0 {
