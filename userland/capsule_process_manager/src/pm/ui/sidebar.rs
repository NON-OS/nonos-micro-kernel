...
use crate::pm::theme::{
    MUTED, NAV_BG_ACTIVE, NAV_BORDER_ACTIVE, NAV_FG, NAV_FG_ACTIVE, SIDEBAR_BG, SIDEBAR_LINE,
    WARNING,
};
...
// Provenance: the refresh counter advances once a second, which is the cheapest
...
fn paint_foot(fb: &mut PaintBuffer, h: u32, refreshes: u32) {
    ...
    let mut buf = [0u8; 12];
    let n = u32_decimal(refreshes, &mut buf);
    let line = y + SIDE_FOOT_TOP + SIDE_FOOT_LINE;
    let advance = text::left(fb, x, line, b"1.0s ", WARNING, BODY_PX);
    text::mono(fb, advance.max(0) as u32, line, &buf[..n], WARNING, BODY_PX);
}
