...
pub fn query(compositor_port: u32, request_id: u32) -> Result<DisplayInfo, &'static str> {
    ...
    let stride = u32::from_le_bytes(body[8..12].try_into().map_err(|_| "display stride missing")?);
    let format = u32::from_le_bytes(body[12..16].try_into().map_err(|_| "display format missing")?);
