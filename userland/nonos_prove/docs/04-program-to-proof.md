...
prove hello.prv   reads a program from the VFS and proves it
```
...
```rust
let report = nonos_prove::prove_source_with_inputs(
...
your purpose. Choosing the right program is your job; proving it ran is prøve's.
...
(`src/driver.rs`, `src/vm.rs`). This is the honest failure mode: prøve will not
