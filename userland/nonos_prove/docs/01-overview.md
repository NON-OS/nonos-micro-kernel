...
prøve is a language whose programs carry a proof of their own execution. You run
...
machine that ran it or you run it yourself. prøve runs a program and hands you a
...
prøve is deliberately smaller, for three reasons that come straight from living
inside an operating system.

The first is a shared field. Every prøve value is a Goldilocks field element, the
`Fp` type from the `nonos-stark` crate (`userland/nonos_prove/src/vm.rs` holds the
...
tree, and no translation between a run and its proof; a prøve trace is already in
the form the prover consumes.

The second is a core small enough to audit by hand. prøve has no data-dependent
...
every constraint in `userland/nonos_prove/src/air.rs` in one sitting.
...
A prøve proof attests a statement of the form: for this public program, on these
