...
Proving is work, and prøve treats it as a paid operation settled in NOX. This page
...
The quote, computed by `quote` in `userland/nonos_prove/src/nox.rs`, is a small
...
check, a game move, an agent's decision. It submits the prøve program and the
...
`nonos_prove`. This page describes the pricing primitive that is built and the
