// NONOS Operating System
// Copyright (C) 2026 NONOS Contributors
//
// This program is free software: you can redistribute it and/or modify
// it under the terms of the GNU Affero General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// This program is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
// GNU Affero General Public License for more details.
//
// You should have received a copy of the GNU Affero General Public License
// along with this program. If not, see <https://www.gnu.org/licenses/>.
use crate::image::jpeg::marker::read_segment_len;
use crate::image::jpeg::mcu::{walk_scan, ScanContext};
use crate::image::jpeg::sos::parse_sos;
use crate::image::types::{DecodeError, ImageSize};

use super::state::DecodeState;
use super::validate_quant_tables::validate_quant_tables;

pub fn handle_sos(
    input: &[u8],
    out: &mut [u32],
    state: &mut DecodeState,
) -> Result<ImageSize, DecodeError> {
    let seg_len = read_segment_len(input, &mut state.pos)?;
    let seg = &input[state.pos..state.pos + seg_len];
    state.pos += seg_len;
    let frame = state.frame_opt.as_ref().ok_or(DecodeError::Unsupported)?;
    let scan = parse_sos(seg, frame)?;
    validate_quant_tables(frame, &state.dqt)?;
    let ctx = ScanContext {
        frame,
        scan: &scan,
        dc_tables: &state.dc_tables,
        ac_tables: &state.ac_tables,
        qt: &state.dqt,
        restart_interval: state.restart_interval,
    };
    walk_scan(&ctx, input, state.pos, out)?;
    ImageSize::new(frame.width as u32, frame.height as u32)
}
