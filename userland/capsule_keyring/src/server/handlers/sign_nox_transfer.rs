...
use super::super::eip1559::{signed_nox_transfer_tx, unsigned_nox_transfer_payload};
...
// Sign transfer(to, amount) on the NOX token: send NOX to another address.
// Ownership-gated like every sign.
pub fn sign_nox_transfer(store: &mut Store, req: Request<'_>, sender_pid: u32) -> Vec<u8> {
    const HDR: usize = 4 + 4 + 32 * 4 + 20 + 32;
    ...
    let mut to = [0u8; 20];
    to.copy_from_slice(&p[136..156]);
    let amount = field32(p, 156);
    let unsigned =
        unsigned_nox_transfer_payload(&nonce, &max_priority, &max_fee, &gas, &to, &amount);
    ...
    let raw = signed_nox_transfer_tx(
        (&nonce, &max_priority, &max_fee, &gas, &to, &amount),
        sig[64] - 27,
        &r,
        &s,
    );
