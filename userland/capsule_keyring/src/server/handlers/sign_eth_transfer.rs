...
use super::super::eip1559::{signed_eth_transfer_tx, unsigned_eth_transfer_payload};
...
pub fn sign_eth_transfer(store: &mut Store, req: Request<'_>, sender_pid: u32) -> Vec<u8> {
    const HDR: usize = 4 + 4 + 20 + 32 * 5;
    ...
    let mut to = [0u8; 20];
    to.copy_from_slice(&p[8..28]);
    let nonce = field32(p, 28);
    let max_priority = field32(p, 60);
    let max_fee = field32(p, 92);
    let gas = field32(p, 124);
    let value = field32(p, 156);
    let unsigned =
        unsigned_eth_transfer_payload(&nonce, &max_priority, &max_fee, &gas, &to, &value);
    ...
    let raw = signed_eth_transfer_tx(
        (&nonce, &max_priority, &max_fee, &gas, &to, &value),
        sig[64] - 27,
        &r,
        &s,
    );
