...
use super::super::eip1559::{signed_nox_stake_tx, unsigned_nox_stake_payload};
...
// Sign stake(amount) on the staking proxy: step two of staking, after the
// approve has confirmed. Ownership-gated like every sign.
pub fn sign_stake(store: &mut Store, req: Request<'_>, sender_pid: u32) -> Vec<u8> {
    ...
    let unsigned = unsigned_nox_stake_payload(&nonce, &max_priority, &max_fee, &gas, &amount);
    ...
    let raw =
        signed_nox_stake_tx((&nonce, &max_priority, &max_fee, &gas, &amount), sig[64] - 27, &r, &s);
