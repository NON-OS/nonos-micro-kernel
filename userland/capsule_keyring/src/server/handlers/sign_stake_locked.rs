...
use super::super::eip1559::{signed_nox_stake_locked_tx, unsigned_nox_stake_locked_payload};
...
// Sign stakeLocked(amount, lockPeriod): the same stake committed for a term.
// The term weights the stake more heavily and cannot be shortened, so it is
// carried explicitly rather than defaulted. Ownership-gated like every sign.
pub fn sign_stake_locked(store: &mut Store, req: Request<'_>, sender_pid: u32) -> Vec<u8> {
    const HDR: usize = 4 + 4 + 32 * 6;
    ...
    let lock = field32(p, 168);
    let unsigned =
        unsigned_nox_stake_locked_payload(&nonce, &max_priority, &max_fee, &gas, &amount, &lock);
    ...
    let raw = signed_nox_stake_locked_tx(
        (&nonce, &max_priority, &max_fee, &gas, &amount, &lock),
        sig[64] - 27,
        &r,
        &s,
    );
