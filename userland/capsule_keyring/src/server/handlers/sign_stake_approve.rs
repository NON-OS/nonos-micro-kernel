...
use super::super::eip1559::{signed_nox_stake_approve_tx, unsigned_nox_stake_approve_payload};
...
// Sign approve(stakingProxy, amount) on the NOX token: step one of staking, so
// the staking contract can pull the staked NOX. Ownership-gated like every sign.
pub fn sign_stake_approve(store: &mut Store, req: Request<'_>, sender_pid: u32) -> Vec<u8> {
    ...
    let unsigned =
        unsigned_nox_stake_approve_payload(&nonce, &max_priority, &max_fee, &gas, &amount);
    ...
    let raw = signed_nox_stake_approve_tx(
