...
pub fn unlink(owner_pid: u32, path: &[u8]) -> Result<(), &'static str> {
    ...
    let (status, _) = super::call::call(peer.port, super::types::OP_UNLINK, 9, &body, &mut rx)?;
    if status != 0 {
        return Err("vfs remove failed");
    }
    Ok(())
}
