...
// Record an access so the path surfaces in a recents listing.
pub fn journal_touch(owner_pid: u32, path: &[u8]) -> Result<(), &'static str> {
    ...
    let (status, _) =
        super::call::call(port, super::types::OP_JOURNAL_TOUCH, 23, &body, &mut rx)?;
    if status != 0 {
        return Err("vfs journal touch failed");
    }
    Ok(())
}
