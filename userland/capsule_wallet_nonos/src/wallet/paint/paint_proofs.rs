...
fn row(fb: &mut PaintBuffer, w: u32, y: u32, hash: &[u8], meta: &str, tag: &[u8], tone: u32) {
    ...
    let _ = fb.text_ttf_mono(368, (y + 12) as i32, hs, FG(), 16.1);
    let _ = fb.text_ttf(368, (y + 34) as i32, meta, MUTED(), 12.6);
    let tw = fb.measure_ttf(core::str::from_utf8(tag).unwrap_or(""), 12.6).max(0) as u32 + 20;
