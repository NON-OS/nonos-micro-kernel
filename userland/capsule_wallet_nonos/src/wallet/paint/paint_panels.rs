...
fn lock_screen(fb: &mut PaintBuffer) {
    ...
    center(fb, cx, 352, "Wallet locked", FG(), 25.3);
    center(fb, cx, 386, "Click anywhere to unlock", MUTED(), 16.1);
}

fn command(fb: &mut PaintBuffer) {
    let x = 410u32;
    ui::card(fb, x, 150, 460, 396);
    let _ = fb.text_ttf((x + 20) as i32, 166, "COMMAND PALETTE", DIM(), 12.1);
    ui::bordered(fb, x + 20, 188, 420, 34, PANEL_2(), LINE());
    let _ = fb.text_ttf((x + 32) as i32, 197, "Type a command\u{2026}", MUTED(), 16.1);
    for (i, cmd) in CMDS.iter().enumerate() {
        let y = 240 + i as u32 * 36;
        let _ = fb.text_ttf((x + 24) as i32, (y + 8) as i32, cmd, FG(), 16.1);
    }
}
...
fn messages(fb: &mut PaintBuffer) {
    ...
    let _ = fb.text_ttf((x + 18) as i32, 98, "NOTIFICATIONS", DIM(), 12.1);
    // No event feed is wired, so the panel does not fabricate one.
    let _ = fb.text_ttf((x + 18) as i32, 132, "No notifications", MUTED(), 14.9);
}

fn account(fb: &mut PaintBuffer, state: &State) {
    let x = 994u32;
    ui::card(fb, x, 82, 260, 96);
    let _ = fb.text_ttf((x + 18) as i32, 98, "ACCOUNT", DIM(), 12.1);
    fb.fill_rect(x + 8, 118, 244, 34, SEL());
    fb.fill_rect(x + 8, 118, 3, 34, ACCENT());
    let _ = fb.text_ttf((x + 20) as i32, 122, "Main", FG(), 16.1);
    if state.address_ready {
        let mut sa = [0u8; 13];
        super::super::hex::short_addr(&state.address, &mut sa);
        let s = core::str::from_utf8(&sa).unwrap_or("");
        let _ = fb.text_ttf_mono((x + 20) as i32, 138, s, MUTED(), 13.2);
    } else {
        let _ = fb.text_ttf((x + 20) as i32, 138, "not generated", MUTED(), 13.2);
    }
}
