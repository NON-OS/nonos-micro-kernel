...
pub fn self_check() -> bool {
    ...
    super::request_chain_id::request_chain_id(1).starts_with(b"{\"jsonrpc\"")
        ...
        && super::request_balance::request_balance(&addr, 2)
            .windows(14)
            .any(|w| w == b"eth_getBalance")
        && super::request_nonce::request_nonce(&addr, 3)
            .windows(23)
            .any(|w| w == b"eth_getTransactionCount")
        && super::request_fee::request_fee(4).windows(12).any(|w| w == b"eth_gasPrice")
        && super::request_broadcast::request_broadcast(&raw, 5)
            .windows(22)
            .any(|w| w == b"eth_sendRawTransaction")
        && super::request_receipt::request_receipt(&hash, 6)
            .windows(25)
            .any(|w| w == b"eth_getTransactionReceipt")
        && super::parse_u64::parse_u64(b"{\"result\":\"0x1\"}") == Some(1)
        && super::parse_hash32::parse_hash32(tx) == Some(hash)
        && super::parse_receipt_ok::parse_receipt_ok(b"{\"result\":{\"status\":\"0x1\"}}")
            == Some(true)
