...
pub fn server_hostname_flight(client: &ClientFlight, bytes: &[u8], host: &[u8]) -> bool {
    ...
    while pos + 5 <= bytes.len() {
        ...
        if bytes[pos] == 23 && decrypt_scan(&ctx, seq, &bytes[pos..end], host) {
            return true;
        }
        pos = end;
        seq += 1;
...
fn decrypt_scan(ctx: &super::server_context::ServerContext, seq: u64, record: &[u8], host: &[u8]) -> bool {
    ...
    super::scan_hostname::scan(msgs, host)
}
