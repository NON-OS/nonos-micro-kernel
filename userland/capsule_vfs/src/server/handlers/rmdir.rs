...
use crate::protocol::{encode_response, Request, EACCES, EINVAL, MAX_PATH_BYTES, OP_RMDIR};
use crate::store::Store;

// Payload: u32 caller_pid, u8 path_len, path bytes, u8 recursive. A non-zero
// recursive byte removes the directory's whole subtree; otherwise a non-empty
// directory returns ENOTEMPTY.
pub fn rmdir(store: &mut Store, req: Request<'_>, sender_pid: u32) -> Vec<u8> {
    let (_pid, rest) = match split_caller(req.payload, sender_pid) {
        Ok(v) => v,
        Err(s) => return encode_response(OP_RMDIR, req.flags, req.request_id, s, &[]),
    };
    if rest.is_empty() {
    ...
    match store.rmdir(&path, recursive) {
        Ok(()) => encode_response(OP_RMDIR, req.flags, req.request_id, 0, &[]),
        Err(e) => encode_response(OP_RMDIR, req.flags, req.request_id, map_store_err(e), &[]),
    }
}
