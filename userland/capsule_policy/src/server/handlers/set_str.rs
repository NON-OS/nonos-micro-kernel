...
pub fn handle(pid: u32, field: Field, payload: &[u8]) {
    ...
    if !set_str::set(field, payload) {
        respond::err(pid, OP_SET, field as u32, KIND_STR, E_INVAL);
        return;
    }
    push::on_string_set(field, payload);
    respond::ok(pid, OP_SET, field as u32, KIND_STR, &[]);
}
